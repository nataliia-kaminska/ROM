...
def ingest_grants_gov(
    ...
    if db is not None:
        return _ingest_grants_gov_in_db(db, keyword=keyword, limit=limit, import_results=import_results)
...
def _ingest_grants_gov_in_db(db, keyword: str, limit: int, import_results: bool) -> GrantsGovIngestionResult:
    ensure_source(
        db,
        name="grants.gov",
    ...
    try:
        hits = GrantsGovClient().search(keyword, limit)
